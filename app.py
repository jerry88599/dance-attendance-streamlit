...
if page == "首页":
    ...
    with col2:
        st.info("🔍 学生追踪\n🗑️ 记录管理\n💾 备份恢复")
    st.divider()
    st.success("请从左侧菜单开始操作 ✅")

# ------------------- 考勤录入 -------------------
elif page == "考勤录入":
    st.title("📝 考勤录入")
    current_class = st.selectbox("选择班级", ["街舞1班", "街舞2班"])
...
# ------------------- 记录管理 -------------------
    ...
    st.title("🗑️ 记录管理")
    d_filter = st.text_input("日期筛选（2026-03）")
    c_filter = st.selectbox("班级", ["", "街舞1班", "街舞2班"])
    records = []
    if os.path.exists(CSV_FILE):
        with open(CSV_FILE, encoding='utf-8') as f:
            for r in csv.DictReader(f):
                if d_filter and not r["日期"].startswith(d_filter): continue
                if c_filter and r["班级"] != c_filter: continue
                records.append(r)
